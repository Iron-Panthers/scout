...
const FieldImageLocations = ({prop, phase, locationProp, popupInfo}) => {
    ...
    const locations = [["1",0.195,0.15],    
      ...
      const dispatchButtonLocation = (location) => {
        ...
        const current = phase ? (state[state.phase] ?? {})[locationProp] : state[locationProp]
    ...
    const renderLocations = ({dimensions, offsets}) => {
          ...
          const currentLocations = phase ? (state[state.phase] ?? {})[locationProp] : state[locationProp]
        ...
        for(const location of locations) {
              ...
              const styles = {
                ...
                left: offsets.x + dimensions.width * location[1] - width / 2,
                top: offsets.y + dimensions.height * location[2] - height / 2,
  ...
  return (
    <FieldInput prop = {prop} phase = {phase} renderChildren = {renderLocations} popupInfo = {popupInfo}/>
